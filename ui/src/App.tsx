...
function App() {
  ...
  useEffect(() => {
    if (documentId) {
      const stuff = fetch("http://localhost:3000/documents/" + documentId)
        .then((e) => {
          if (!e.ok) {
            navigate("/");
          }
  ...
  const save = async () => {
    let resp = await fetch("http://localhost:3000/documents", {
      method: "POST",
      body: content,
    });
  ...
  return (
    <div className="dark:bg-gray-900 text-white h-screen flex flex-col">
      ...
      <div className="p-2 flex flex-row justify-between">
        ...
        <span className="text-gray-600">The Programmer's Hangout</span>
      </div>
    </div>
  );
}
